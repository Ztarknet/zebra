...
impl RpcClient for RpcRequestClient {
    ...
    async fn get_raw_transaction(
        ...
        // Try verbose=1 first (works for both mempool and blockchain txs)
        match self.json_result_from_call("getrawtransaction", format!(r#"["{txid_hex}", 1]"#))
            .await
        {
            Ok(response) => Ok(response),
            Err(_) => {
                // Fallback to verbose=0 if verbose=1 fails
    ...
    async fn get_address_utxos_with_mempool(
        ...
        info!("Found {} confirmed UTXOs, {} mempool transactions", 
                  confirmed_utxos.len(), mempool_tx_ids.len());
        ...
        let target_address: transparent::Address = address.parse()
            ...
            for input in tx.transparent_bundle().iter().flat_map(|b| b.vin.iter()) {
                ...
                for utxo in &confirmed_utxos {
                    ...
                    if outpoint_hash_bytes == utxo_hash_bytes && outpoint_index == utxo_index {
                        let utxo_key = format!("{}:{}", hex::encode(utxo.txid().0), utxo.output_index().index());
                    ...
                    if let Some(addr) = output.recipient_address() {
                        ...
                        let addr_zebra: transparent::Address = match addr {
                            zcash_transparent::address::TransparentAddress::PublicKeyHash(hash) => {
                                transparent::Address::from_pub_key_hash(zebra_chain::parameters::Network::new_default_testnet().kind(), hash)
                            }
                            zcash_transparent::address::TransparentAddress::ScriptHash(hash) => {
                                transparent::Address::from_script_hash(zebra_chain::parameters::Network::new_default_testnet().kind(), hash)
                            }
                        };
        ...
        info!("Filtered {} spent UTXOs, {} mempool UTXOs created, {} total UTXOs", 
                  filtered_count, mempool_utxos.len(), confirmed_utxos.len() + mempool_utxos.len());
